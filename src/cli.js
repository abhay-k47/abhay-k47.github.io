...
const commands = {};
let systemData = {};
const rootPath = "users/abhaykk/root";
...
commands.cat = (filename) => {
  ...
  const hasValidFileExtension = (filename, extension) =>
    filename.includes(extension);
  const isFileInDirectory = (filename) =>
    filename.split("/").length === 1 ? false : true;
  const isFileInSubdirectory = (filename, directory) =>
    struct[directory].includes(filename);
  ...
  if (isFileInDirectory(filename)) {
    if (hasValidFileExtension(filename, ".txt")) {
      ...
      const fileKey = directories
        .slice(1, directories.length)
        .join(",")
        .split(".")[0];
